...
const getSignedUploadUrl = async (req, res) => {
  ...
  const s3key = `${generateUUID()}.${ext}` // this works for png, jpg, pdf, ...
  ...
  const options = {
    signatureVersion: 'v4',
    region: config.region, // same as your bucket
    // endpoint: new aws.Endpoint(`${config.bucket_name}.s3-accelerate.amazonaws.com`), 
